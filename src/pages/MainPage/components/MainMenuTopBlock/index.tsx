import MenuModule from '@modules/MenuModule/containers/MenuModule';
import { DigitalCategorySelection } from '@src/pages/DigitalPage/components/DigitalCategorySelection';
import { TurboCategorySelection } from '@src/pages/TurboPage/components/TurboCategorySelection';
import React from 'react';
import { useLocation } from 'react-router-dom';
import { CategorySelection } from '../CategorySelection';
import './styles.scss';

const MainMenuTopBlock = () => {

	const locations = useLocation();
	const {pathname} = locations;

	return (
		<div style={ { overflow: 'hidden', minHeight: pathname.includes('course') ? 'auto' : '760px' } } className="animation-block__wrapper">
			<div className="animation-block__top">
				<div className="animation-block__topBackground" />

				<MenuModule />
				{ pathname === '/komus_digital' && <CategorySelection /> }
				{ pathname === '/komus_digital/digital_knowledge' && <DigitalCategorySelection /> }
				{ pathname === '/komus_digital/turbo_knowledge' && <TurboCategorySelection /> }
			</div>

			<div className="animation-block__slogan">
        		<img className="animation-block__sloganImage" src={`${process.env['API']}/komus_num/app/images/who_are_you_1.svg" alt=""`} title=""/>
      		</div>    
			  
			{/* <div className="animation-block__slogan">
				<span className='animation-block__sloganImage'>
					<WhoAreYouSvg/>
				</span>
			</div> */}
